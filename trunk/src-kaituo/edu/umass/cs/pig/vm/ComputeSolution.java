...
import edu.umass.cs.pig.dataflow.PltAggrRelation;
...
//        	}
        	...
        	/*
        	 * This if branch solves the case:
        	 * pigServer.registerQuery("AN = load " + neighbors
				...
				when we solve AN, the constraints (e.g. objID1 < objID2) have nothing to do with
				any of AN's fileds;  but in D, AN's fields need to equal to objID1.  This is an indirect
				equivalence relation.  We need to pay attention to these indirect equivalence in advance. 
        	 */
        	if(pl.children.size() != 0) {
				List<Relation> childrenPL = pl.children;
				
				for(Relation r : childrenPL) {
					if(r instanceof PltAggrRelation)
						;
					else {
						Map<VarInfo, VarInfo> childParentMap = r.child_to_parent_map;
						Set<VarInfo> keySet = childParentMap.keySet();
...
//            }
	...
	public void compute(PathList pl, Tuple exampleTuple, QueryManager q, LogicalSchema schema, 
			DataBag newInputData, DataBag inputData, List<LogicalSchema> allSchema,
			Iterator<Map.Entry<PathList, LoadArgs>> it,
			Map<PathList, LoadArgs> childrenPathlistMap,
			Map<PathList, ParentEncapsulation> parentTupleMap,
			Map<PathList, ParentEncapSol> parentSolMap) throws FrontendException {
		boolean newInput = false;
...
//    	}
		...
		for(Relation parent: pl.parents) {
			if(childrenPathlistMap.containsKey(parent.parent)) 
				return;
		}
...
//                	} else {
                    	...
                    	if(!inputTuple.synthetic) {
                        	TupleMutation tm = new TupleMutation(schema, allSchema);
                    		Tuple newInputTuple = tm.mutate(inputTuple);
                    		ExampleTuple newExample = new ExampleTuple(newInputTuple);
                    		penc = new ParentEncapsulation(newExample, schema);
                    		if(MainConfig.debug) {
                        		// Generate concrete value by sending constraints to a constraint solver  
                                System.out.println(newExample.toString());
