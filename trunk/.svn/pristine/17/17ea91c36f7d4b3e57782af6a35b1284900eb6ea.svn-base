...
abstract class AlgebraicMathBase<T> extends EvalFunc<T> implements Algebraic {
    ...
    private final class KnownOpProviderNested implements KnownOpProvider, Serializable {
		/**
		 * 
		 */
		private static final long serialVersionUID = -3308038491587068501L;
		private final KNOWN_OP op;

		private KnownOpProviderNested(KNOWN_OP op) {
			this.op = op;
		}
    ...
    protected void setOp(final KNOWN_OP op) {
        opProvider = new KnownOpProviderNested(op);
    }
