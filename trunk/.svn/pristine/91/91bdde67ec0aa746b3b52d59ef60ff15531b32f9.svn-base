...
public class EqualVarCntr extends PltRelCntr implements Serializable {
	...
	public EqualVarCntr(Map<VarInfo,VarInfo> mp, byte ty) {
		super(ty, ty);
		equalVars = mp;
	}
	...
	public SWIGTYPE_p__Z3_ast asZ3ast(Z3Context z3, Allocator4Z3 alloc) {
		...
		while(it.hasNext()) {
			...
			ConstraintResult lhsAst = alloc.mallocInZ3(keyName, key.getType(), -1);//, fl
			ConstraintResult rhsAst = alloc.mallocInZ3(valueName, value.getType(), -1);//, fl
