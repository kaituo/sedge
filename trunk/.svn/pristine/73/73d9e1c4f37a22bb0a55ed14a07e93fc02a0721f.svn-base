...
public class TestExGen4FilterConditionNum16 {
...
//		filePhotoObj.deleteOnExit();
	...
	public void testFilter16() throws Exception {
		...
		pigServer.registerQuery("G1 = load " + GalaxyPair3
				+ " using PigStorage() as (objID1 : long, ra : float, dec : float, modelMag_u1 : float, modelMag_g : float, modelMag_r : float, modelMag_i : float, modelMag_z : float, petroR50_r : float, petrorad_u1 : float, petrorad_g1 : float, petrorad_r1 : float, petrorad_i1 : float, petrorad_z1 : float, petroRadErr_g1 : float, petroMag_g1 : float);");
		pigServer.registerQuery("G2 = load " + GalaxyPair4
				+ " using PigStorage() as (objID2 : long, ra : float, dec : float, modelMag_u2 : float, modelMag_g : float, modelMag_r : float, modelMag_i : float, modelMag_z : float, petroR50_r : float, petrorad_u2 : float, petrorad_g2 : float, petrorad_r2 : float, petrorad_i2 : float, petrorad_z2 : float, petroRadErr_g2: float, petroMag_g2 : float);");
		...
		pigServer.registerQuery("G = filter F by petroMag_g2 >= 16.0 and petroMag_g2 <= 21.0 and modelMag_u1 > -9999.0;");
