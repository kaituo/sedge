...
public class TestExGen4Filter {
    ...
    public void testFilter6() throws Exception {
        ...
        String query = "A = load " + C
                + " using PigStorage() as (x : chararray, y : int);\n";
        pigserver.registerQuery(query);
        query = "B = filter A by x matches '.*apache.*';";
        pigserver.registerQuery(query);
        Map<Operator, DataBag> derivedData = pigserver.getExamples2("B");
        assertTrue(derivedData != null);
    }
    ...
    public void testFilter7() throws Exception {
        ...
        String query = "A = load " + C
                + " using PigStorage() as (x : chararray, y : int);\n";
        pigserver.registerQuery(query);
        query = "B = filter A by x matches '.*apache.*' AND y < 11;";
        pigserver.registerQuery(query);
        Map<Operator, DataBag> derivedData = pigserver.getExamples2("B");
        assertTrue(derivedData != null);
    }
    ...
    public void testFilter9() throws Exception {
        ...
        String query = "A = load " + C
                + " using PigStorage() as (x : chararray, y : int);\n";
        pigserver.registerQuery(query);
        query = "B = filter A by x is not null;";
        pigserver.registerQuery(query);
        Map<Operator, DataBag> derivedData = pigserver.getExamples2("B");
        assertTrue(derivedData != null);
    }
