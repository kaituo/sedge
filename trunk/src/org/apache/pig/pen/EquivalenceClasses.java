    ...
    private static void getEqClasses(PhysicalPlan plan, Operator parent, LogicalPlan lp,
        ...
        if (parent instanceof LOForEach) {
            ...
            if (eqClasses == null) {
                ...
                //Kaituo: add a case to check if logToPhyMap.get(parent) is an instanceof org.apache.pig.backend.hadoop.executionengine.physicalLayer.relationalOperators.POUnion
                if(logToPhyMap.get(parent) instanceof POPackage) {
                	...
                	for (Entry<PhysicalOperator, Collection<IdentityHashSet<Tuple>>> entry : poToEqclassesMap.entrySet()) {
                        PhysicalOperator op = entry.getKey();
                        if(op instanceof POUnion)
                        	eqClasses.addAll(entry.getValue());
                    }
                }    
            }
            result.put((LogicalRelationalOperator)parent, eqClasses);
