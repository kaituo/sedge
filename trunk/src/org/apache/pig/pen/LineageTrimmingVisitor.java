...
public class LineageTrimmingVisitor extends LogicalRelationalNodesVisitor {
    ...
    // Kaituo: for left outer join only for now
    ...
    public LineageTrimmingVisitor(LogicalPlan plan,
            ...
            PhysicalPlan physPlan, PigContext pc, boolean isouterjoin) throws IOException, InterruptedException {
    ...
    public void visit(LOJoin join) throws FrontendException {
    	// Kaituo: we don't want to any data records removed if outerjoin is set;
    	// otherwise, the trimming visitor would remove some interesting data
    	// records
//    	if(isOuterJoin)
//    		return;
        if (continueTrimming) {
    ...
    /**
     * Kaituo: this is a temporary workaround to ignore split here
     *  because split is ignored in physical plan level.  If we don't ignore,
     *  there will be nullpointer exception:
	    	
	    java.lang.NullPointerException
		at org.apache.pig.pen.LineageTrimmingVisitor.PruneBaseDataConstrainedCoverage(LineageTrimmingVisitor.java:230)
		at org.apache.pig.pen.LineageTrimmingVisitor.checkCompleteness(LineageTrimmingVisitor.java:491)
		at org.apache.pig.pen.LineageTrimmingVisitor.processOperator(LineageTrimmingVisitor.java:445)
		at org.apache.pig.pen.LineageTrimmingVisitor.visit(LineageTrimmingVisitor.java:213)
	*/
    @Override
...
//            processOperator(split);
    ...
    private Map<LOLoad, DataBag> PruneBaseDataConstrainedCoverage(
        ...
        // Kaituo: 
        // This is my previous comment: bad hack, ignoring best lineage group when leftoutjoin is spotted
        // Right now, I don't remember why I have such conclusion. So comment out the changes I made before
//        if(isOuterJoin) {
//        	for (Tuple lineageGroup : lineageGroupToEquivClasses.keySet()) {
...
//        }
    ...
    /**
     * Kaituo:
     * Lineage here is: {D: (Name: LOSplitOutput Schema: id1#21:int,id2#22:int)=org.apache.pig.pen.util.LineageTracer@1fe1281}
     * op is: 1-2: (Name: LOSplit Schema: id1#5:int,id2#6:int).  So no lineage for LOSplit.  "1-2" is its alias. 
     * maybe because LOSplit is a dummy operator and translates to nothing in physical plan and no lineage info is recorded.
     * @param op
     * @return
     * @throws Exception
     */
