...
public class ExampleGenerator {
    ...
    //Kaituo
	List<LogicalSchema>  oldLOADSchema = new ArrayList<LogicalSchema>(); 
...
//                .getProperties());
 ...
 //       pigContext.setDefaultLogLevel(Level.FATAL);
    ...
    public Map<Operator, DataBag> getExamples() throws IOException, InterruptedException {
        ...
        // delete hashLimit and the following part
        // would make illustrator not working well
     ...
     * @author kaituo
     */
	private void saveOldLOADSchema() {
		List<Operator> loads = plan.getSources();
		for (Operator load : loads) {
			LogicalSchema schema;// uidOnlySchema;
			LOLoad ld = (LOLoad)load;
	    	try {
				schema = ld.getSchema();
				LogicalSchema copied = (LogicalSchema)DeepCopy.copy(schema);
				oldLOADSchema.add(copied);
//				List<LogicalFieldSchema> fieldSchema = schema.getFields();
//				for(LogicalFieldSchema f: fieldSchema) {
//					LogicalFieldSchema copied = (LogicalFieldSchema)DeepCopy.copy(f);
//					oldLOADSchema.add(copied);
//				}
    ...
    /**
     * In the following step, Pig will compile logical plan to physical plan and do some optimization in
     * the middle.  You can see the optimization in the file "a", "b", "c" in the same package of this
     * class.  The main difference 1) LOAD's schema all change to bytearray; 2) another FOREACH operator
     * is inserted immediately after original LOAD and do some type conversion.  I don't know why Pig wants
     * to do this.  I try to do: save schema of original LOAD before it is changed.
     * Otherwise, if I extract the schema when I need it,  it actually
     * extracts the new schema of LOAD (all bytearray). 
     * 
     * physPlan = compilePlan(plan);
     * 
     * @return
     ...
     * @author kaituo
     */
    public Map<Operator, DataBag> getExamples2() throws IOException, InterruptedException {
        if (pigContext.getProperties().getProperty("pig.usenewlogicalplan", "true").equals("false"))
            throw new ExecException("ILLUSTRATE must use the new logical plan!");
        pigContext.inIllustrator = true;
        pigContext.inGetExamples2 = true;
        ...
        // "Got new derived data from the trimmed base data");
        // augment base data
        InvertConstraintVisitor augment = new InvertConstraintVisitor(newPlan,
                logToPhyMap, baseData, derivedData, oldLOADSchema);
        augment.visit();
        this.baseData = augment.getNewBaseData();
        // System.out.println("Obtained augmented base data");
        // create new derived data and trim the base data after augmenting
        ...
        //0419
//        trimmer = new LineageTrimmingVisitor(newPlan, baseData, this,
//                logToPhyMap, physPlan, pigContext, augment.isIsouterjoin());
//        trimmer.visit();
        //0419
//        baseData = trimmer.getBaseData();
        // System.out.println("Final trimming");
...
//        }
        ...
        System.out.println(DisplayExamples.printTabular(newPlan,
                derivedData, forEachInnerLogToDataMap));
        pigContext.inIllustrator = false;
        pigContext.inGetExamples2 = false;
    ...
    private void readBaseData(List<Operator> loads) throws IOException, InterruptedException, FrontendException, ExecException {
        ...
        //Kaituo: make it nonnull before calling getData
        baseData = null;
//        if(oldLOADSchema.size() != 0)
//        	baseData = new HashMap<LOLoad, DataBag>();
//        addBaseData4Loads(loads);
        Map<Operator, DataBag> result = getData(thisPhyPlan);
        // Kaituo: put the initialization of basedData before calling getData because I want to put some data inside fist
        baseData = new HashMap<LOLoad, DataBag>();
    ...
    private void readBaseData2(List<Operator> loads) throws IOException, InterruptedException, FrontendException, ExecException {
        PhysicalPlan thisPhyPlan = new PhysicalPlan();
        for (Operator op : loads) {
            LogicalSchema schema = ((LOLoad) op).getSchema();
            if(schema == null) {
...
//        baseData = new HashMap<LOLoad, DataBag>();
    ...
    //Kaituo: in case of empty file
    private void addBaseData4Loads(List<Operator> loads) throws IOException, InterruptedException, FrontendException, ExecException {
    	for (Operator op : loads) {
            LogicalSchema schema = ((LOLoad) op).getSchema();
            DefaultTuple dt = new DefaultTuple(schema, oldLOADSchema);
            Tuple defaultInputT = dt.newDefaultTuple();
            DataBag loadInputData = BagFactory.getInstance().newDefaultBag();
            loadInputData.add(defaultInputT);
            baseData.put((LOLoad) op, loadInputData);
    	}
    }
...
//    /**
//     * @author kaituo
//     * Terminate connection to theorem prover.
//     *
//     * <p>Call this method just before exiting Dsc.
//     */
//    public void exitZ3() {
//    	Z3Context.get().del_context();
//    }
}
