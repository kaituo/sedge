...
//			log.warn("No inputs found. Signaling End of Processing.");
    ...
    private Result getNext() throws ExecException {
    	if(funcSpec.getClassName().equals("org.apache.pig.builtin.COUNT")) {
    		...
    		InputOutput io = new InputOutput(); //Kaituo
        	
            Result result = processInput();
            if(!funcSpec.getClassName().equals("org.apache.pig.builtin.COUNT")) {
            	io.setInput((Result)(DeepCopy.copy(result))); // Kaituo
            }
            ...
            try {
                if(result.returnStatus == POStatus.STATUS_OK) {
                    if (isAccumulative()) {
                        ...
                        result.result = func.exec((Tuple) result.result);
                        io.setOutput((Result)(DeepCopy.copy(result))); // Kaituo
                        }
                    }
                   
                    UserFuncValue.get().put(funcSpec, io);
                    return result;
                }
