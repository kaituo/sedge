...
//    private Z3Context z4 = Z3Context.get();
    ...
    public void testFilter2() throws Exception {
        ...
        String query = "A = load " + A
                + " using PigStorage() as (x : int, y : int);\n";
        pigserver.registerQuery(query);
        query = "B = filter A by x > 5 AND x < 20;";
        pigserver.registerQuery(query);
        Map<Operator, DataBag> derivedData = pigserver.getExamples("B");
    ...
    public void testFilter4() throws Exception {
        ...
        String query = "A = load " + A
                + " using PigStorage() as (x : int, y : int);\n";
        pigserver.registerQuery(query);
        query = "B = filter A by x > 5;";
        pigserver.registerQuery(query);
        //query = "C = filter B by TRUE AND TRUE;";
        //pigserver.registerQuery(query);
        Map<Operator, DataBag> derivedData = pigserver.getExamples("B");
    ...
    public void testJoin4() throws IOException, ExecException {
        ...
        pigServer.registerQuery("A1 = load " + B + " as (x, y);");
        pigServer.registerQuery("B1 = load " + A + " as (x, y);");
